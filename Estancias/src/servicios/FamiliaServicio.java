...
public class FamiliaServicio {
    ...
    public void crearFamilia(int id, String nombre, int edadMinima, int edadMaxima, int numHijos, String email) throws Exception {
        // Validaciones
        if (nombre == null || nombre.trim().isEmpty()) throw new IllegalArgumentException("El nombre no puede estar vacío.");
        if (edadMinima <= 0 || edadMaxima <= 0 || edadMinima > edadMaxima) throw new IllegalArgumentException("Edades inválidas.");
        if (numHijos < 0) throw new IllegalArgumentException("El número de hijos no puede ser negativo.");
